...
in the future.
  The main entry point of the pipeline 'exome-seek' calls this job 
submission wrapper script. As so, this script can be used to by-pass 
'exome-seek' for a previously failed run; meaning, it can be used to 
re-run the pipeline to pick back off where the last failure occurred
or re-start the pipeline.
  Please Note: it is highly recommended to use 'exome-seek'; it is the 
main entry point and preferred entry point of the exome-seek pipeline. 
If you are experiencing an error, it maybe due to improperly mounting 
singularity bind paths which 'exome-seek' will internally handle.
  ...
  [1] MODE  [Type: Str] Defines the snakemake execution mode.
                        Valid mode options include: <slurm, ...>
                          slurm: uses slurm and singularity/snakemake
                             ...
                             running exome-seek in this mode as most 
...
Required Arguments:
  ...
  -b, --bind-paths [Type:Path]  Singularity bind paths. The pipeline uses
                                 ...
                                 or by-passing exome-seek, you will need
...
Example:
  $ runner slurm -h
  $ runner slurm -j exome-seek_hg38 -b "/data/$USER/,/lscratch"
